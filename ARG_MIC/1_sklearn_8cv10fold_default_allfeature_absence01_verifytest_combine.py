...
verify_testdata=pd.read_csv("RS_811/14880file_ARGanno_Protein_result_absence01_addPhenotype_" + arguments[1] + "_verify_test.txt",sep='\t', header=0,index_col=0)
...
for fold, (train_idx, val_idx) in enumerate(skf.split(X_train, y_train)):
    ...
    fold_train_df.to_csv(f"AllFeature/XGBoost_ARG_absence01_fold_{fold+1}_train_samples_10fold_"+ arguments[1] +".txt", sep='\t', index=True, header=True)
    fold_val_df.to_csv(f"AllFeature/XGBoost_ARG_absence01_fold_{fold+1}_val_samples_10fold_"+ arguments[1] +".txt", sep='\t', index=True, header=True)
...
all_metrics_df.to_csv("AllFeature/XGBoost_10fold_crossvalidation_default_hyperparameter_ARG_absence01_"+ arguments[1] +".txt", sep='\t', index=True, header=True)
