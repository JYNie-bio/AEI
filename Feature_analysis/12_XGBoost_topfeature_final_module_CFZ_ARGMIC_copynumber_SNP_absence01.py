...
traindata=pd.read_csv("~/ARG_with_SNP/RS_811/14880file_ARGanno_copynumber_SNPmatrix959090_absence01_addPhenotype_CFZ_train.txt",sep='\t', header=0,index_col=0, keep_default_na=False, dtype=CategoricalDtype())
verifytestdata=pd.read_csv("~/ARG_with_SNP/RS_811/14880file_ARGanno_copynumber_SNPmatrix959090_absence01_addPhenotype_CFZ_verify_test.txt",sep='\t', header=0,index_col=0, keep_default_na=False, dtype=CategoricalDtype())
...
df_sorted=pd.read_csv("~/ARG_with_SNP/AllFeature/XGBoost_10fold_crossvalidation_default_hyperparameter_ARGanno_copynumber_SNPmatrix959090_absence01_CFZ_fold_2_feature_importances_category_and_combine_verifytest.txt",sep='\t', header=0,index_col=0)
df_filtered = df_sorted[df_sorted['Importance'] > 0]
df_filtered_top_index0 = df_filtered.head(30).index
...
old_col_pattern = "aminoglycoside_6\'-N-acetyltransferase_AAC\(6\'\)-Iy"
traindata2.rename(columns={col: re.sub(old_col_pattern, "aminoglycoside_N-acetyltransferase_AAC(6')-Iy", col)
                              for col in traindata2.columns}, inplace=True)
verifytestdata2.rename(columns={col: re.sub(old_col_pattern, "aminoglycoside_N-acetyltransferase_AAC(6')-Iy", col)
                              for col in verifytestdata2.columns}, inplace=True)
traindata2.to_csv("~/Feature_analysis/use_topfeature/XGBoost_CFZ_topfeature_finalmodel_traindata.txt", sep='\t', index=True, header=True)
verifytestdata2.to_csv("~/Feature_analysis/use_topfeature/XGBoost_CFZ_topfeature_finalmodel_verifytestdata.txt", sep='\t', index=True, header=True)
...
old_col_pattern = "aminoglycoside_6\'-N-acetyltransferase_AAC\(6\'\)-Iy"
X_train2.rename(columns={col: re.sub(old_col_pattern, "aminoglycoside_N-acetyltransferase_AAC(6')-Iy", col)
                              for col in X_train2.columns}, inplace=True)
X_verify_test2.rename(columns={col: re.sub(old_col_pattern, "aminoglycoside_N-acetyltransferase_AAC(6')-Iy", col)
                              for col in X_verify_test2.columns}, inplace=True)
...
df.to_csv("~/Feature_analysis/use_topfeature/XGBoost_CFZ_topfeature_finalmodel_evaluation_indicator_results.txt", sep='\t', index=True, header=True)
...
df0_sorted.to_csv("~/Feature_analysis/use_topfeature/XGBoost_CFZ_topfeature_finalmodel_feature_importances.txt", sep='\t', index=True, header=True)
...
verify_test_metrics = calculate_metrics(y_verify_test, y_verify_test_pred, y_verify_test_pred_proba)
df = pd.DataFrame([verify_test_metrics],index=['XGBoost_verify_test'])
df.to_csv("~/Feature_analysis/use_topfeature2/XGBoost_CFZ_topfeature_finalmodel_evaluation_indicator_results.txt", sep='\t', index=True, header=True)
...
df0_sorted = df0.sort_values(by='Importance', ascending=False)
df0_sorted.to_csv("~/Feature_analysis/use_topfeature2/XGBoost_CFZ_topfeature_finalmodel_feature_importances.txt", sep='\t', index=True, header=True)
