...
traindata=pd.read_csv("~/ARG_with_SNP/RS_811/14880file_ARGanno_absence01_SNPmatrix959090_absence01_addPhenotype_CAZ_train.txt",sep='\t', header=0,index_col=0, keep_default_na=False, dtype=CategoricalDtype())
verifytestdata=pd.read_csv("~/ARG_with_SNP/RS_811/14880file_ARGanno_absence01_SNPmatrix959090_absence01_addPhenotype_CAZ_verify_test.txt",sep='\t', header=0,index_col=0, keep_default_na=False, dtype=CategoricalDtype())
...
df_sorted=pd.read_csv("~/ARG_with_SNP/AllFeature/XGBoost_10fold_crossvalidation_default_hyperparameter_ARGanno_absence01_SNPmatrix959090_absence01_CAZ_fold_1_feature_importances_category_and_combine_verifytest.txt",sep='\t', header=0,index_col=0)
...
traindata2.to_csv("~/Feature_analysis/use_topfeature/XGBoost_CAZ_topfeature_finalmodel_traindata.txt", sep='\t', index=True, header=True)
verifytestdata2.to_csv("~/Feature_analysis/use_topfeature/XGBoost_CAZ_topfeature_finalmodel_verifytestdata.txt", sep='\t', index=True, header=True)
...
model = XGBClassifier(learning_rate=0.01,max_depth=8,n_estimators=1000,random_state=2, enable_categorical=True)
...
df.to_csv("~/Feature_analysis/use_topfeature/XGBoost_CAZ_topfeature_finalmodel_evaluation_indicator_results.txt", sep='\t', index=True, header=True)
...
df0_sorted.to_csv("~/Feature_analysis/use_topfeature/XGBoost_CAZ_topfeature_finalmodel_feature_importances.txt", sep='\t', index=True, header=True)
...
verify_test_metrics = calculate_metrics(y_verify_test, y_verify_test_pred, y_verify_test_pred_proba)
df = pd.DataFrame([verify_test_metrics],index=['XGBoost_verify_test'])
df.to_csv("~/Feature_analysis/use_topfeature2/XGBoost_CAZ_topfeature_finalmodel_evaluation_indicator_results.txt", sep='\t', index=True, header=True)
...
df0_sorted = df0.sort_values(by='Importance', ascending=False)
df0_sorted.to_csv("~/Feature_analysis/use_topfeature2/XGBoost_CAZ_topfeature_finalmodel_feature_importances.txt", sep='\t', index=True, header=True)
