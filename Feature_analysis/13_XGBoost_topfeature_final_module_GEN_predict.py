...
predictdata=pd.read_csv("predict_use_data/Salmonella_enterica_all_checkmPass_20231128_prokkapath_info_haveyear_havecountry_2014_arg_last_use_result_last_GEN_topfeature_matrix.txt",sep='\t', header=0,index_col=0, keep_default_na=False, dtype=CategoricalDtype())
...
df = pd.DataFrame([predictdata2.index, data_pred2], index=['Genome', 'GEN_predict_phenotype'])
df_transposed=df.T
df_transposed.to_csv("predict_use_data/Salmonella_enterica_all_checkmPass_20231128_prokkapath_info_haveyear_havecountry_2014_arg_last_use_result_last_GEN_topfeature_predict_result.txt", sep='\t', index=False, header=True)
