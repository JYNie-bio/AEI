...
if arguments[2] == "1":
    ...
    verify_testdata=pd.read_csv("RS_811/14880file_ARGanno_copynumber_SNPmatrix959090_absence01_addPhenotype_" + arguments[1] + "_verify_test.txt",sep='\t', header=0,index_col=0, keep_default_na=False, dtype=CategoricalDtype())
...
for fold, (train_idx, val_idx) in enumerate(skf.split(X_train, y_train)):
    ...
    fold_train_df.to_csv(f"AllFeature/XGBoost_ARGanno_copynumber_SNPmatrix959090_absence01_fold_{fold+1}_train_samples_10fold_category_and_combine_verifytest_"+ arguments[1] +".txt", sep='\t', index=True, header=True)
    fold_val_df.to_csv(f"AllFeature/XGBoost_ARGanno_copynumber_SNPmatrix959090_absence01_fold_{fold+1}_val_samples_10fold_category_and_combine_verifytest_"+ arguments[1] +".txt", sep='\t', index=True, header=True)
...
all_metrics_df.to_csv("AllFeature/XGBoost_10fold_crossvalidation_default_hyperparameter_ARGanno_copynumber_SNPmatrix959090_absence01_category_and_combine_verifytest_"+ arguments[1] +".txt", sep='\t', index=True, header=True)
