...
#### 8:2 split train\test
data_all<-read.table("~/ARG_with_SNP/14880file_ARGanno_copynumber_SNPmatrix959090_absence01",tempstr,"_addPhenotype.txt",header = T,sep="\t",quote = '',comment.char = '',check.names = F)
...
write.table(data_arg3_test,paste0("14880file_ARGanno_copynumber_SNPmatrix959090_absence01_addPhenotype_",colnames(data_arg)[ncol(data_arg)],"_verify_test.txt"),sep="\t",quote=F,row.names = T,col.names = T)
...
write.table(data_arg3_2_train,paste0("14880file_ARGanno_copynumber_SNPmatrix959090_absence01_addPhenotype_",colnames(data_arg)[ncol(data_arg)],"_train.txt"),sep="\t",quote=F,row.names = T,col.names = T)
