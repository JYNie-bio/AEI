...
#### 8:2 split train\test
data_all<-read.table("~/ARGSNP/RefSeq_Anno_Result_14880file_addinfo_add_prokka_arg_fromdiamond_havearg_last_use_result_last_SNPmatrix959090_YesNoNA",tempstr,"_addPhenotype.txt",header = T,sep="\t",quote = '',comment.char = '',check.names = F)
...
write.table(data_arg3_test,paste0("RefSeq_Anno_Result_14880file_addinfo_add_prokka_arg_fromdiamond_havearg_last_use_result_last_SNPmatrix959090_YesNoNA_addPhenotype_",colnames(data_arg)[ncol(data_arg)],"_verify_test.txt"),sep="\t",quote=F,row.names = T,col.names = T)
...
write.table(data_arg3_2_train,paste0("RefSeq_Anno_Result_14880file_addinfo_add_prokka_arg_fromdiamond_havearg_last_use_result_last_SNPmatrix959090_YesNoNA_addPhenotype_",colnames(data_arg)[ncol(data_arg)],"_train.txt"),sep="\t",quote=F,row.names = T,col.names = T)
