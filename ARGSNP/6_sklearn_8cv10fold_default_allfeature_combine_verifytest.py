...
arguments = sys.argv
new_working_directory = "~/ARGSNP/"
os.chdir(new_working_directory)

traindata=pd.read_csv("RS_811/RefSeq_Anno_Result_14880file_addinfo_add_prokka_arg_fromdiamond_havearg_last_use_result_last_SNPmatrix959090_addPhenotype_" + arguments[1] + "_train.txt",sep='\t', header=0,index_col=0)
verifytestdata=pd.read_csv("RS_811/RefSeq_Anno_Result_14880file_addinfo_add_prokka_arg_fromdiamond_havearg_last_use_result_last_SNPmatrix959090_addPhenotype_" + arguments[1] + "_verify_test.txt",sep='\t', header=0,index_col=0)
...
for fold, (train_idx, val_idx) in enumerate(skf.split(X_train, y_train)):
    ...
    fold_train_df.to_csv(f"AllFeature/XGBoost_SNP959090_absence01_fold_{fold+1}_train_samples_10fold_combine_verifytest_"+ arguments[1] +".txt", sep='\t', index=True, header=True)
    fold_val_df.to_csv(f"AllFeature/XGBoost_SNP959090_absence01_fold_{fold+1}_val_samples_10fold_combine_verifytest_"+ arguments[1] +".txt", sep='\t', index=True, header=True)
...
all_metrics_df.to_csv("AllFeature/XGBoost_10fold_crossvalidation_default_hyperparameter_SNP959090_absence01_combine_verifytest_"+ arguments[1] +".txt", sep='\t', index=True, header=True)
